...
def parse_args():
    ...
    parser.add_argument('--book_no', default='0001', type=str)
...
def query_chaps(book_no):
    ...
    return len(editer.get_chap_list()[0])
...
def downloader_router(root_path,
    ...
    #     # query_chaps(book_no)
    #     # 返回目錄長度
    #     num_volume  = query_chaps(book_no)
    #     return 
    # elif "-" in volume_no:
    elif volume_no == '':
        # 自動化下載多卷，首先取得書籍目錄信息，之後再進行下載
        volume_no = '1-'
        print(book_no)
        temp_volume_no = volume_no+str(query_chaps(book_no))
        # print(temp_volume_no)
        start, end = map(str, temp_volume_no.split("-"))
...
if __name__=='__main__':
    ...
    # downloader_router(root_path='out', book_no=str(2))
    for i in range(21, 100):
        downloader_router(root_path='out', book_no=str(i))
    # if args.no_input:
